...
const int DetailedResolutionClass::MaxTiming = TIMING_GTF_STANDARD;
...
const int DetailedResolutionClass::MaxHBack[] = {4093, 65534, 65534};
const int DetailedResolutionClass::MinHBlank[] = {2, 2, 2};
const int DetailedResolutionClass::MaxHBlank[] = {4095, 65536, 65536};
const int DetailedResolutionClass::MinHTotal[] = {3, 3, 3};
const int DetailedResolutionClass::MaxHTotal[] = {8190, 131072, 131072};
...
const int DetailedResolutionClass::MaxVBack[] = {4093, 65534, 65534};
const int DetailedResolutionClass::MinVBlank[] = {2, 2, 2};
const int DetailedResolutionClass::MaxVBlank[] = {4095, 65536, 65536};
const int DetailedResolutionClass::MinVTotal[] = {3, 3, 3};
const int DetailedResolutionClass::MaxVTotal[] = {8190, 131072, 131072};

const long long DetailedResolutionClass::MinVRate[] = {0, 0, 0};
const long long DetailedResolutionClass::MaxVRate[] = {72816666666LL, 18641351111111LL, 1864135111111LL};
const long long DetailedResolutionClass::MinHRate[] = {0, 0, 0};
const long long DetailedResolutionClass::MaxHRate[] = {218450000LL, 55924053333LL, 5592405333LL};
...
bool DetailedResolutionClass::GetText(char *Text, int TextSize, const char *Dash)
{
	...
	std::snprintf(Text, TextSize, "%dx%d%s @ %lld.%03lld Hz (%lld.%0*lld MHz) [%s/%s]%s%s", HActive, VActive, Interlaced ? "i" : "", ActualVRate / 1000, ActualVRate % 1000, PClock / PClockPrecision[Type], GetPClockDigits(), PClock % PClockPrecision[Type], HPolarity ? "+" : Dash, VPolarity ? "+" : Dash, Stereo ? " (3D)" : "", Preferred ? "*" : "");
	return true;
}
//---------------------------------------------------------------------------
bool DetailedResolutionClass::SetType(int NewType)
{
	...
	if (PClock != DECIMAL_BLANK)
	{
		PClock = PClock * PClockPrecision[Type] / PClockPrecision[OldType];
...
bool DetailedResolutionClass::CopyPossible()
{
	...
	if (CopyPClock == PClock * 1000 / PClockPrecision[Type])
...
bool DetailedResolutionClass::Copy()
{
	...
	CopyStereo = Stereo;
	CopyPClock = PClock * 1000 / PClockPrecision[Type];
...
bool DetailedResolutionClass::PastePossible()
{
	...
	if (PClock == (CopyPClock * PClockPrecision[Type] + 999) / 1000)
...
bool DetailedResolutionClass::Paste()
{
	...
	PClock = (CopyPClock * PClockPrecision[Type] + 999) / 1000;
...
bool DetailedResolutionClass::UpdateInterlaced()
{
	...
	else
	{
		...
		{
			if (Positive(VActive) && VActive % 2 != 0)
				VActiveI = VActive * 2;
...
bool DetailedResolutionClass::CalculateNative(bool Digital)
{
	...
	else
	{
		...
		PClock = PClock / Multiple * Multiple;
	}
...
bool DetailedResolutionClass::CalculateAutomaticPC()
{
	...
	if (AutomaticPC[Index].HActive != 0)
	{
		...
		if (IsValid() || !IsValidHActive() || !IsValidVActive() || !IsValidPClock())
			return IsSupported();
	}
...
bool DetailedResolutionClass::CalculateExact()
{
	...
	HFront = 48;
	HSync = 32;
	HBack = (HActive + HFront + HSync + 80) / 80 * 80 - HActive - HFront - HSync;
	VFront = GetVFrontForCVT();
	VSync = GetVSyncForCVT();
	VBack = (VActive + VFront + VSync + 125) / 125 * 125 - VActive - VFront - VSync;
	CalculateHBlank();
	CalculateHTotal();
	CalculateVBlank();
	CalculateVTotal();
...
bool DetailedResolutionClass::CalculateExactReduced()
{
	...
	if ((HActive + HFront + HSync + HBack) % 400 == 0 || VRate % 5000 == 0)
		VBack = (VActive + VFront + VSync + 25) / 25 * 25 - VActive - VFront - VSync;
	else
		VBack = (VActive + VFront + VSync + 125) / 125 * 125 - VActive - VFront - VSync;
	...
	if (HActive * VActive <= 2457600 * Double)
	{
		if (PClock > 165 * PClockPrecision[Type])
		{
			HFront = 24;
			HSync = 32;
			HBack = (HActive + HFront + HSync + 80) / 80 * 80 - HActive - HFront - HSync;
...
long long DetailedResolutionClass::GetHPeriodForCVT()
{
	...
	return (1000000000000000LL * Double / VRate - MinTimeCVT * Double) / (VActive + GetVFrontForCVT() + Interlaced);
}
//---------------------------------------------------------------------------
long long DetailedResolutionClass::GetHPeriodForCVTRB()
{
	...
	return (1000000000000000LL * Double / VRate - MinTimeCVTRB * Double) / VActive;
}
//---------------------------------------------------------------------------
long long DetailedResolutionClass::GetHPeriodForCVTRB2()
{
	...
	return (1000000000000000LL * Double / VRate - MinTimeCVTRB2 * Double) / VActive;
}
//---------------------------------------------------------------------------
long long DetailedResolutionClass::GetHPeriodForGTF()
{
	...
	return (1000000000000000LL * Double / VRate - MinTimeGTF * Double) / (VActive + GetVFrontForGTF() + Interlaced);
}
//---------------------------------------------------------------------------
...
int DetailedResolutionClass::GetVFrontForCVTRB2()
{
	...
	int VFront = VBlank * Double + Interlaced - GetVSyncForCVTRB2() - GetVBackForCVTRB2();
...
int DetailedResolutionClass::GetVSyncForCVT()
{
	...
	for (int Index = 0; AspectVSync[Index][0] != 0; Index++)
		if (Aspect >= AspectVSync[Index][0] && Aspect <= AspectVSync[Index][1])
			return AspectVSync[Index][2] * Double;
...
int DetailedResolutionClass::GetVBackForCVT()
{
	...
	int VBack = VSyncVBack * Double + Interlaced - GetVSyncForCVT();

	if (VBack < 6 * Double + Interlaced)
		VBack = 6 * Double + Interlaced;
...
int DetailedResolutionClass::GetVBackForCVTRB()
{
	...
	int VBack = VBlank * Double + Interlaced - GetVFrontForCVTRB() - GetVSyncForCVTRB();
...
int DetailedResolutionClass::GetVBackForGTF()
{
	...
	int VBack = VSyncVBack * Double + Interlaced - GetVSyncForGTF();
/*
	if (VBack < 6 * Double + Interlaced)
		VBack = 6 * Double + Interlaced;
*/
...
{
	return 1000000000 * Double / PClockPrecision[Type];
}
//---------------------------------------------------------------------------
...
bool DetailedResolutionClass::CalculatePClockForCVT()
{
	...
	int Multiple = PClockPrecision[Type] / 4;
	PClock = HTotal * Multiplier / HPeriod / Multiple * Multiple;
...
bool DetailedResolutionClass::CalculatePClockForCVTRB()
{
	...
	int Multiple = PClockPrecision[Type] / 4;
	PClock = VRate * HTotal * VTotal / GetVRateDivisor() / Multiple * Multiple;
...
//---------------------------------------------------------------------------
bool DetailedResolutionClass::CalculatePClockForCVTRB2()
{
	if (!IsSupportedVRate() || !IsSupportedHTotal() || !IsSupportedVTotal())
	{
		PClock = DECIMAL_BLANK;
	...
	PClock = VRate * HTotal * VTotal / GetVRateDivisor();
...
bool DetailedResolutionClass::CalculatePClockForGTF()
{
	...
	PClock = (VRate * HTotal * VTotal + Divisor / 2) / Divisor;
...
bool DetailedResolutionClass::CalculatePClockFromVRate()
{
	...
	PClock = (VRate * HTotal * VTotal + (Divisor - 1)) / Divisor;
...
bool DetailedResolutionClass::CalculatePClockFromHRate()
{
	...
	PClock = (HRate * HTotal + (Divisor - 1)) / Divisor;
...
bool DetailedResolutionClass::CalculateActualVRate()
{
	...
	ActualVRate = PClock * GetVRateDivisor() / HTotal / VTotal;
...
bool DetailedResolutionClass::CalculateActualHRate()
{
	...
	ActualHRate = PClock * GetHRateDivisor() / HTotal;
	...
	if (Timing == 0)
	{
		switch (LastRate)
		{
			...
			case 1:
				...
				if (!IsSupportedVTotal())
					return IsValidHRate() && IsValidPClock() && IsValidActualHRate();
...
{
	int ClampHFront = Clamp(HFront, MinHFront[Type], MaxHFront[Type]);
	int ClampHSync = Clamp(HSync, MinHSync[Type], MaxHSync[Type]);
	return Min(MaxHBack[Type], MaxHBlank[Type] - ClampHFront - ClampHSync);
}
//---------------------------------------------------------------------------
int DetailedResolutionClass::GetMinHBlank(int Type)
{
	int ClampHFront = Clamp(HFront, MinHFront[Type], MaxHFront[Type]);
	int ClampHSync = Clamp(HSync, MinHSync[Type], MaxHSync[Type]);
	return Max(MinHBlank[Type], ClampHFront + ClampHSync + MinHBack[Type]);
}
//---------------------------------------------------------------------------
...
{
	int ClampHActive = Clamp(HActive, MinHActive[Type], MaxHActive[Type]);
	int ClampHFront = Clamp(HFront, MinHFront[Type], MaxHFront[Type]);
	int ClampHSync = Clamp(HSync, MinHSync[Type], MaxHSync[Type]);
	return Max(MinHTotal[Type], ClampHActive + ClampHFront + ClampHSync + MinHBack[Type]);
}
//---------------------------------------------------------------------------
int DetailedResolutionClass::GetMaxHTotal(int Type)
{
	...
	int ClampHActive = Clamp(HActive, MinHActive[Type], MaxHActive[Type]);
	return Min(MaxHTotal[Type], ClampHActive + MaxHBlank[Type]);
}
//---------------------------------------------------------------------------
...
{
	int ClampVFront = Clamp(GetValue(VFront), MinVFront[Type], MaxVFront[Type]);
	int ClampVSync = Clamp(GetValue(VSync), MinVSync[Type], MaxVSync[Type]);
	return Min(MaxVBack[Type], MaxVBlank[Type] - ClampVFront - ClampVSync);
}
//---------------------------------------------------------------------------
int DetailedResolutionClass::GetMinVBlank(int Type)
{
	int ClampVFront = Clamp(GetValue(VFront), MinVFront[Type], MaxVFront[Type]);
	int ClampVSync = Clamp(GetValue(VSync), MinVSync[Type], MaxVSync[Type]);
	return Max(MinVBlank[Type], ClampVFront + ClampVSync + MinVBack[Type]);
}
//---------------------------------------------------------------------------
...
{
	int ClampVActive = Clamp(GetValue(VActive), MinVActive[Type], MaxVActive[Type]);
	int ClampVFront = Clamp(GetValue(VFront), MinVFront[Type], MaxVFront[Type]);
	int ClampVSync = Clamp(GetValue(VSync), MinVSync[Type], MaxVSync[Type]);
	return Max(MinVTotal[Type], ClampVActive + ClampVFront + ClampVSync + MinVBack[Type]);
}
//---------------------------------------------------------------------------
int DetailedResolutionClass::GetMaxVTotal(int Type)
{
	...
	int ClampVActive = Clamp(GetValue(VActive), MinVActive[Type], MaxVActive[Type]);
	return Min(MaxVTotal[Type], ClampVActive + MaxVBlank[Type]);
}
//---------------------------------------------------------------------------
int DetailedResolutionClass::Min(int Value1, int Value2)
{
	if (Value1 <= Value2)
		return Value1;
...
//---------------------------------------------------------------------------
int DetailedResolutionClass::Max(int Value1, int Value2)
{
	if (Value1 >= Value2)
		return Value1;
...
//---------------------------------------------------------------------------
int DetailedResolutionClass::Clamp(int Value, int Min, int Max)
{
	if (Value == BLANK || Value == INVALID)
		return Min;
